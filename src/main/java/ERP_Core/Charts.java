 ...
 * Charts can show best-seller products or products with the greatest revenue
...
public class Charts {
                        ...
                        do {
                            try {
                                ...
                                fillData_BestSellers((AnalyzeData.sortSalesPerMonth(
                                        AnalyzeData.getSalesOfYear(ProductSale.getAllSales(), year1))).get(month));
                                break;
                            } catch (java.util.InputMismatchException e) {
    ...
    public static void fillData_BestSellers(ArrayList<ProductSale> sales) {
        try {
            System.out.printf("%-8s %-15s %3s    %-12s%-8s", "Rank", "Product Name", "ID", "Quantity", "BarChart");
            System.out.println();
            ...
            for (int i = 0; i < bestSellersQuants.length; i++) {
                System.out.printf("%2d.      %-15s %3d    %-12d", i + 1, bestSellersNames[i], bestSellersQuants[i][0], bestSellersQuants[i][1]);
    ...
    public static void fillData_BestRevenue(ArrayList<ProductSale> sales) {
        try {
            System.out.printf("%-8s %-15s %3s    %-12s%8s", "Rank", "Product Name", "ID", "Revenue", "BarChart");
            System.out.println();
            ...
            for (int i = 0; i < bestRevenueValues.length; i++) {
                System.out.printf("%2d.      %-15s %3.0f    %-12.2f", i + 1, bestRevenueNames[i], bestRevenueValues[i][0], bestRevenueValues[i][1]);
    ...
    public static void chartBestPerMonthQuant() {
        ...
        do {
            try {
                ...
                ArrayList<ArrayList<ProductSale>> salesOfEachMonth = AnalyzeData.sortSalesPerMonth(AnalyzeData.getSalesOfYear(ProductSale.getAllSales(), year));
                ...
                for (int i = 0; i < 12; i++) {
                    ...
                    System.out.printf("%-15s %-15s %3d    %-5s", AnalyzeData.getMonth(i), product.getName(), bestProductOfMonth[i][0], bestProductOfMonth[i][1]);
                    System.out.println();
                }
            } catch (java.util.InputMismatchException e) {
                System.out.println("Wrong input. Year must be an integer number. Try again...");
